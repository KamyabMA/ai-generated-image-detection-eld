...
from utils.datasets_and_dataloaders import index_dataloader_helper
from approach_eld.eld_testing import evaluate_model_per_patch
...
def training_loop(wandb_project_name,
                  train_index_dataloader,
                  eval_index_dataloader,
                  meta_data_for_index_dataloaders,
                  train_ids_for_index_dataloaders,
    ...
    iters_per_epoch = len(train_index_dataloader)
    ...
    with wandb.init(project=wandb_project_name, name=run_name) as run:
        ...
        for epoch in mb:
            ...
            for iteration, (batch_of_ids) in enumerate(
                progress_bar(iter(train_index_dataloader), parent=mb)):

                # get image patches and their labels
                images, labels = index_dataloader_helper(train_ids_for_index_dataloaders,
                                                        meta_data_for_index_dataloaders,
                                                        batch_of_ids,
                                                        patch_size,
                                                        sampling_method,
                ...
                if scheduler:
                    ...
                    lr = learning_rate
                run.log({'training loss': loss, 'epoch': epoch, 'learning rate': lr},
            ...
            evaluation = evaluate_model_per_patch(eval_index_dataloader,
                                                meta_data_for_index_dataloaders,
                                                eval_ids_for_index_dataloaders,
                                                patch_size,
                                                sampling_method,
