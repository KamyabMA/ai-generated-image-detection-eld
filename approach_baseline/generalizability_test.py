...
from utils.datasets_and_dataloaders import create_dataloader
from models import Model_DINOv2S_1_LN
from approach_baseline.training import training_loop
...
def train_on_one_dataset_test_on_rest(train_dataset: str,
    ...
    train_loader = create_dataloader(train_ids, batch_size, train_meta_data_path, shuffle=True, num_workers=5)
    eval_loader = create_dataloader(eval_ids, batch_size, train_meta_data_path, shuffle=False, num_workers=5)
    ...
    for i in range(len(all_datasets_name)):
        ...
        test_loader = create_dataloader(test_ids, batch_size, all_datasets_meta_data_paths[i], shuffle=False, num_workers=5)
        
        if not os.path.exists(test_result_save_dir):
            os.makedirs(test_result_save_dir)
        test_result_save = f"{test_result_save_dir}/trained_on_{train_dataset}_tested_on_{all_datasets_name[i]}_output.json"
        evaluation = evaluate_model(test_loader,
                                  model,
                                  device=device)
        output_dict = {}
        output_dict["accuracy_score"] = evaluation["accuracy_score"]
        output_dict["precision_score"] = evaluation["precision_score"]
        output_dict["recall_score"] = evaluation["recall_score"]
        output_dict["f1_score"] = evaluation["f1_score"]
        output_dict["roc_auc_score"] = evaluation["roc_auc_score"]
        json_object = json.dumps(output_dict, indent=4)
        with open(test_result_save, "w") as outfile:
            outfile.write(json_object)


if __name__ == "__main__":
    model = Model_DINOv2S_1_LN()
    test_result_save_dir = "approach_baseline/output/Config_DINOv2S_1_LN_3/exp_generalizability_test"
    wandb_per_run_suffix = "DINOv2S_1_LN_3"
    wandb_project_name = "GT_BASELINE"
